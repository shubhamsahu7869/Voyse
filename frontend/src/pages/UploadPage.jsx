...
export default function UploadPage() {
 const [files, setFiles] = useState([]); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const navigate = useNavigate();
 ...
 const generate = async () => {
  ...
  try {
   ...
   setError(err.response?.data?.message || "Could not process these documents. Please try again.");
   setLoading(false);
  }
 };
 ...
 {error && <p className="form-error">{error}</p>}<div className="upload-foot"><span><Lock size={14} /> Your documents are sent securely over HTTPS.</span><button className="button" disabled={!files.length || loading} onClick={generate}>{loading ? <><Sparkles className="spin" /> Composing itinerary...</> : <>Create my itinerary <ArrowRight size={17} /></>}</button></div></main></>;
}
